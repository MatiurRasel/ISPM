...
}) =>
{
    ...
    addComponents(
        _.fromPairs(_.map(options.themes, (theme, themeName) => [
            ...
            ))), (value, key) => [[`--ctd-${e(key)}`, value], [`--ctd-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
        ])),
    );
    ...
    const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) =>
    {
        ...
        const background = theme(`ctd.customProps.background.${colorScheme}`);
        const foreground = theme(`ctd.customProps.foreground.${colorScheme}`);
        ...
        return {
            [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                ...
                ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--ctd-${e(key)}`, value], [`--ctd-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--ctd-${e(key)}`, value], [`--ctd-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
            },
        };
    });
        ...
        theme: {
            extend: {
                ...
                colors: _.fromPairs(_.flatten(_.map(_.keys(flattenColorPalette(normalizeTheme(options.themes.default))), (name) => [
                    [name, `rgba(var(--ctd-${name}-rgb), <alpha-value>)`],
                    [`on-${name}`, `rgba(var(--ctd-on-${name}-rgb), <alpha-value>)`],
                ]))),
            },
            ctd  : {
                customProps: {
